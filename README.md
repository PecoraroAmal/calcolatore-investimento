...
Un'applicazione web progettata per calcolare e confrontare strategie di investimento basate su due opzioni: **Standard** e **Premium**. L'obiettivo è aiutare gli utenti a simulare il rendimento di un investimento e identificare la combinazione ottimale per massimizzare il guadagno finale su un periodo specificato in anni.
...
- **Input validati**: Controlli rigorosi sui valori inseriti (es. anni ≥ 1, saldo iniziale tra 1000 e 10.000.000 €).
...
- **Ottimizzazione computazionale**: Per periodi superiori a 20 anni (240 mesi), utilizza un calcolo approssimativo per evitare sovraccarico.
...
2. Compila il form con i seguenti parametri:
   ...
   - **Anni**: Durata totale dell'investimento in anni (es. 1, 2, 20).
...
- **Mesi**: Durata totale in mesi, calcolata come `Anni * 12` (es. 2 anni = 24 mesi).
- **Periodi per Anno**: Numero di rinnovi in un anno, calcolato come `12 / Mesi per Rinnovo` (es. 12 / 3 = 4).
- **Tasso di Interesse per Periodo Standard**: Tasso annuo diviso per il numero di periodi per anno (es. 0,05 / 4 = 0,0125).
- **Tasso di Interesse per Periodo Premium**: Tasso annuo + 0,001 diviso per il numero di periodi per anno (es. (0,05 + 0,001) / 4 = 0,01275).
- **Numero Totale di Periodi**: Numero totale di rinnovi, calcolato come `Math.floor(Mesi / Mesi per Rinnovo)` (es. 24 / 3 = 8).
...
- **Per Anni ≤ 20 (240 mesi)**: Calcola tutte le possibili combinazioni binarie (2^Anni). Es. per 2 anni: "00", "01", "10", "11".
- **Per Anni > 20**: Calcola solo combinazioni specifiche (es. "000...0" a "111...1" e viceversa) per ridurre il carico computazionale, con un avviso di approssimazione.
...
- **Nota Informativa**: Aggiunta sotto i risultati per chiarire che si tratta di una simulazione, non di consulenza finanziaria.
...
**Tabella Iniziale**:
- Gennaio: -, Febbraio: 100, Marzo: 110, Aprile: 120, Maggio: 100, Giugno: 110, Luglio: 120, Agosto: 100, Settembre: 110, Ottobre: 120, Novembre: 100, Dicembre: 110

**Combinazione "0"**:
1. Periodo 1: Saldo = 5000 → Interesse = 62,5 → Netto = 46,25 → Saldo = 5376,25 (aggiunge 330)
...
**Modifica Manuale**:
- Cambia Giugno da 110 a 130 → `renewalSavingsList`: [330, 340, 330, 330]
- Totale Risparmi: 1330 €
- Il calcolo si aggiorna di conseguenza.

## Limitazioni
- **Calcolo approssimativo per > 20 anni**: Non tutte le combinazioni vengono calcolate per evitare tempi di elaborazione eccessivi.
